...
│   │   ├── permissions-token.ts  # Permissions token parsing and middleware
│   │   ├── token-date.ts  # Shared token date validation
...
│   │   ├── item-stats.ts  # Item-level aggregated stats handler
...
│   │   ├── task-stats.ts  # Task stats route (permissions token)
...
│   │   ├── rest-responses.ts
│   │   └── stats.ts       # Statistical utility functions (median, average, etc.)
...
- **Task Stats Routes** (`/sls/task-stats`):
  - `GET /` - Return aggregated item-level statistics (user count, median time spent, median time to validate, score distribution over time buckets). Requires a permissions token with `can_edit >= "all"` or `is_owner`. Data sourced from `UserTaskStats`, excluding records with `missingEarlierActivity`.
- **Common Routes**:
...
#### Permissions Token Module (`src/auth/permissions-token.ts`)

Domain-specific token parsing for item permission-based operations:
- **Token Sources**: HTTP Authorization header (Bearer) only
- **Token Origin**: Generated by AlgoreaBackend to assert a user's permissions on a specific item
- **Token Payload**:
  - `user_id`, `item_id`: Identify the user and item
  - `can_view`, `can_grant_view`, `can_watch`, `can_edit`: Permission levels (ordered enums)
  - `is_owner`: Whether the user owns the item
  - `date`: Token date in `dd-mm-yyyy` format (must be yesterday/today/tomorrow UTC)
  - `exp`: Expiration as Unix timestamp
- **Functions**:
  - `parsePermissionsToken(token, publicKey)`: Validates signature, checks date and exp, transforms to PermissionsToken
- **Middleware**: `requirePermissionsToken` attaches parsed token to `req.permissionsToken`, requires `can_edit >= "all"` or `is_owner`
- **Usage**: Used by task stats endpoint (`GET /task-stats`)
